...
exports.RecoveryKitSingleton = void 0;
const driveEndpoints_1 = require("./driveEndpoints");
const shamirSecret_1 = require("./shamirSecret");
...
class RecoveryKit {
    constructor() {
        ...
        this.splitKey = async (params) => {
            const res = await (0, shamirSecret_1.split)(params.n, params.k, params.secret);
            return res.map((r) => JSON.stringify(r));
        };
        this.combineKey = (params) => {
            return (0, shamirSecret_1.join)(params.keys.map((key) => JSON.parse(key)));
        };
        this.uploadKeyToDrive = async (params) => {
            try {
                let folderId = undefined;
                const { token, folderName, fileName, secretPart } = params;
                const getFolderRes = await (0, driveEndpoints_1.getFolder)({
                    token,
                });
                if (getFolderRes.find((folder) => folder.name === folderName)) {
                    ...
                    const createFolderRes = await (0, driveEndpoints_1.createFolder)({
                        token,
                        folderName,
                    });
                ...
                if (folderId) {
                    ...
                    const getFilesRes = await (0, driveEndpoints_1.getFile)({ token });
                    if (getFilesRes.find((file) => file.name === fileName)) {
                        fileId = getFilesRes.find((file) => file.name === fileName).id;
                    }
                    else {
                        const createFileRes = await (0, driveEndpoints_1.createFile)({
                            token,
                            fileName,
                            folderId,
                        });
                    ...
                    if (fileId) {
                        await (0, driveEndpoints_1.updateFileData)({ token, fileId, body: secretPart });
                    }
                    else {
        ...
        this.uploadKeyToAWS = async (params) => {
            ...
            const userId = await (0, driveEndpoints_1.getUserIdFromAccessToken)({ token });
        ...
        this.recoveryKeyFromDrive = async (params) => {
            try {
                ...
                if (folderId) {
                    ...
                    if (fileId) {
                        const getFileDataRes = await (0, driveEndpoints_1.getFileData)({ token, fileId });
        ...
        this.phraseToEntropy = bip39_1.phraseToEntropy;
        this.entropyToPhrase = bip39_1.entropyToPhrase;
        this.config = {};
    }
}
exports.RecoveryKitSingleton = new RecoveryKit();
